...
p = GRN.plot_3d_pca(trainX, cell_clusters.k5)
filepath = joinpath(GRN.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "cell clusters k5.html")
savefig(p, filepath)
...
CSV.write("/media/yuehhua/Workbench/workspace/GRN/results/cdgrn_k5/cdgrn_k5-2.csv", context_cor)

## Effective gene set
c = context_cor[context_cor.regulation_strength .≥ 0.3, :]
gene_set = DataFrame(gene=unique!(vcat(c.tf, c.target)))
CSV.write("/media/yuehhua/Workbench/workspace/GRN/results/cdgrn_k5/cdgrn_k5-2_gene_set.csv", gene_set)


# sg = GRN.build_graph(c)
# GRN.network_entropy(sg)
