...
GRN.clustermap(D, cell_clusters.cell, method="single", filename="clustermap_true_regulations_soft_clst (total)")
...
context_cor = GRN.cor(tfs, prof, pairs, cell_clusters.k5)
context_pairs = collect(zip(context_cor.tf, context_cor.target))
context_ρs, selected_context = GRN.max_cor(context_cor, k)
context_cor[!, :context] = context_ρs


# global

global_ρs = GRN.global_cor(tfs, prof, context_pairs, map(x -> x.size, context_ρs))
...
spliced_ρs = GRN.spliced_cor(tfs, prof, context_pairs, cell_clusters.k5, selected_context)
...
p = GRN.plot_3d_pca(trainX, cell_clusters.k15)
filepath = joinpath(GRN.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "cell clusters k15.html")
savefig(p, filepath)
...
context_cor = GRN.cor(cdgrn, tfs, prof, context)
...
CSV.write("/media/yuehhua/Workbench/workspace/GRN/results/cdgrn_k3-3.csv", context_cor)

# nodelabel = unique!(vcat(context_cor.tf, context_cor.target))
# g = GRN.to_graph(context_cor, nodes=nodelabel)

# using GraphRecipes
# p = graphplot(g, curves=false, names=nodelabel, nodeshape=:ellipse)
# filepath = joinpath(GRN.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "CDGRN k3-3.png")
# savefig(p, filepath)
...
trainX = Array(df[:, 3:end])'
p = GRN.plot_3d_pca(trainX, df.cell, context)
...
data = make_vis_data(target, tfs, prof, true_regulations, cluster=context)
p = plot_regulations(data, "E2f1", target, model=cdgrn.models[Symbol(target)])
filepath = joinpath(GRN.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "k15-9 regulation $target.html")
savefig(p, filepath)
