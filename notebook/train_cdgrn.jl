...
dir = joinpath(CDGRNs.PROJECT_PATH, "results")
prof = load_data(dir)
...
CDGRNs.clustermap(D, cell_clusters.cell, method="single", filename="clustermap_true_regulations_soft_clst (total)")
...
context_cor = CDGRNs.cor(tfs, prof, pairs, cell_clusters.k5)
...
# filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "CDGRN k3-3.png")
# savefig(p, filepath)
...
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "k3-3 regulation $target.html")
savefig(p, filepath)
...
data = make_vis_data(target, tfs, prof, true_regulations, cluster=context)
p = plot_regulations(data, "E2f1", target, model=cdgrn.models[Symbol(target)])
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "tf-gene gmm model", "CDGRN", "k15-9 regulation $target.html")
savefig(p, filepath)
