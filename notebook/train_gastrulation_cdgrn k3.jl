...
p = CDGRNs.plot_3d_pca(Array(df[:, 3:end])', cell_clusters.k3)
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "cell clusters k3.html")

context = cell_clusters.k3 .== 1
p = CDGRNs.plot_2d_pca(Array(df[:, 3:end])', df.cell, context)
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "cell clusters k3-1.png")
savefig(p, filepath)
...
xticks!([1:3;], cdgrn_stats[!,:cntx])
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "network_size k3.png")
savefig(p, filepath)

p = @df cdgrn_stats plot(:order, :entropy,
    xlabel="Context", ylabel="Network entropy", legend=false)
xticks!([1:3;], cdgrn_stats[!,:cntx])
filepath = joinpath(CDGRNs.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "network_entropy k3.png")
savefig(p, filepath)
