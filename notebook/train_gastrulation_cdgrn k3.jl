...
cell_clusters = DataFrame(cell=prof.obs.celltype, time=prof.obs.latent_time)
...
k = 3
cell_clusters.k3 = cutree(hc_col; k=k)
...
p = CDGRN.plot_3d_pca(trainX, cell_clusters.k3)
...
filepath = joinpath(CDGRN.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "cell clusters k3-2.html")
savefig(p, filepath)
...
# Blood progenitorto erythroid

cortable = Dict()
for c in [1, 2, 3]
    context = cell_clusters.k3 .== c
    ...
    CSV.write(joinpath(dir, "cdgrn_k3", "cdgrn_k3-$(c).csv"), context_cor)

    ## Effective gene set
    correlated = context_cor[context_cor.reg_stng .≥ 0.3, :]
    gene_set = DataFrame(gene=unique!(vcat(correlated.tf, correlated.target)))
    CSV.write(joinpath(dir, "cdgrn_k3", "cdgrn_k3-$(c)_gene_set.csv"), gene_set)
...
CSV.write(joinpath(dir, "cdgrn_k3", "cdgrn_k3_all.csv"), joined)
