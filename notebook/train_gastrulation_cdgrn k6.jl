...
filepath = joinpath(CDGRN.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "cell clusters k6.html")

context = cell_clusters.k6 .== 1
p = CDGRN.plot_2d_pca(Array(df[:, 3:end])', df.cell, context)
filepath = joinpath(CDGRN.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "cell clusters k6-1.png")
savefig(p, filepath)
...
filepath = joinpath(CDGRN.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "network_size k6.png")
savefig(p, filepath)

p = @df cdgrn_stats plot(:order, :entropy,
    xlabel="Context", ylabel="Network entropy", legend=false)
xticks!([1:6;], cdgrn_stats[!,:cntx])
filepath = joinpath(CDGRN.PROJECT_PATH, "pics", "gastrulation_erythroid", "CDGRN", "network_entropy k6.png")
savefig(p, filepath)
